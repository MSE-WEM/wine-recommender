...
Pour ce faire, deux sources de données sont utilisées :
...
Afin de traiter une quantité de données raisonnable, nous nous limitons aux plats principaux pour les recettes et aux vins rouges et blancs pour les vins. Il y a 1'843 vins (1'257 rouges et 586 blancs) et il y a 3'398 recettes. De base, il y avait ~17'000 recettes. En revanche, comme le temps de calcul afin de créer les embeddings pour faire la recommandation durait trop longtemps, nous avons décidé de prendre uniquement 1 recette sur 5.

Nous avons effectué différents prétraitements. Lors de la récupération des données sur les sites que nous avons choisis, il a fallu faire des opérations pour enlever certains caractères (comme les parenthèses ou certaines balises). Il a aussi fallu enlever tous les mots inutiles (de, des, la, le, ...) car nous ne les voulons pas pour créer l'embedding des similarités. Pour les recettes, nous avons aussi enlevé tous les mots qui apparaissaient moins de 5 fois dans toutes les recettes prises en compte, parce qu'il n'y avait pas d'intérêt à avoir ces mots pour l'embedding.

Voici les données des recettes ainsi que leur descriptions :
...
Voici les données des vins ainsi que leur descriptions :
- **name**: le nom du vin
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/pairanything.png?raw=true" align="center" width="50%">
</p>
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/architecture.png?raw=true" align="center" width="50%">
</p>
...
On utilise la librairie Mongoose, dédiée à la connexion aux bases de données de type MongoDB.

### React

La dernière partie de l'architecture est le "frontend". Ce dernier est développé en React avec une interface Material UI.

### CI/CD

Enfin, pour faciliter la mise en production de cette architecture, on utilise une pipeline GitHub Actions. Cette dernière est composée de deux parties. La première pour le "backend" et la DB et la deuxième pour le "frontend".
...
Et pour le "frontend", on build simplement l'application React avant de la *dockeriser* elle aussi.
...
- webapp-wine-recommender.kube.isc.heia-fr.ch (React)
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/base_interface.png?raw=true" align="center" width="50%">
</p>
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/wine_filters.png?raw=true" align="center" width="50%">
</p>
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/ingredient_filters.png?raw=true" align="center" width="50%">
</p>

On peut donc en sélectionner jusqu'à ce qu'il n'en propose plus et cela nous donnera une seule recette à sélectionner.

<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/recipe_after_filters.png?raw=true" align="center" width="50%">
</p>
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/loading_recipes.png?raw=true" align="center" width="50%">
</p>
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/recipes_list.png?raw=true" align="center" width="50%">
</p>

On peut ensuite commencer à écrire et la liste va s'affiner. 

<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/recipes_search.png?raw=true" align="center" width="50%">
</p>
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/wine_recommendation.png?raw=true" align="center" width="50%">
</p>
...
Dans notre projet, nous avons utilisé trois techniques différentes :
- **Le scrapping**: pour récupérer les données que l'on a besoin sur les deux sites internets
- **Système de recommandation avec du "word embedding"**: pour pouvoir suggérer des vins en fonction d'une recette
- **L'analyse de sentiments**: pour analyser la "qualité" du vin en fonction des commentaires d'un vin

### Scrapping

Nous avons utilisé trois librairies pour scrapper les données : **scrapy**, **selenium** et **BeaufitulSoup**. Scrapy est une librairie en Python qui permet de récupérer automatiquement des données à partir de site Web. BeautifulSoup est aussi une librairie en Python qui permet d'extraire des données de site Web. Nous avons utilisé BeautifulSoup lorsque Scrapy n'arrivait pas à faire ce que nous voulions. Ces deux librairies ont été utilisés pour récupérer les données des recettes du site Web Marmiton.

Selenium est aussi une librairie en Python qui permet d'automatiser l'interaction avec les navigateurs Web. Elle permet de simuler des actions humaines, ce qui nous permet, par exemple, de naviguer sur la page. Nous avons dû utiliser cette librairie pour récupérer les données du site Vivino, car nous avions besoin de descendre en bas de la page pour charger les données dynamiquement.

### Système de recommandation avec du "word embedding"
...
Dans le cadre de ce projet, nous avons utilisé cette technique pour recommander des vins en fonction d'une recette. Pour ce faire, nous avons généré des embeddings pour les recettes et les vins en utilisant la librairie [Gensim](https://radimrehurek.com/gensim/) et le modèle [GoogleNews-vectors-negative300](https://drive.google.com/file/d/0B7XkCwpI5KDYNlNUTTlSS21pQmM/edit?resourcekey=0-wjGZdNAUop6WykTtMip30g). GoogleNews-vectors-negative300 est un ensemble de vecteurs de mots pré-entraînés qui ont été créés à partir de Google News en utilisant le modèle Word2Vec. Ces vecteurs représentent les mots sous forme de nombres réels dans un espace vectoriel de 300 dimensions. Nous avons dû choisir ce modèle, car nous n'avons pas trouvé de modèle spécifique aux recettes. Gensim est une bibliothèque Python pour le traitement de texte et la modélisation de sujets à grande échelle, qui permet de créer des modèles de traitement de texte à partir de données brutes et de vecteurs de mots pré-entraînés, avec une grande flexibilité pour la personnalisation des paramètres.

Pour créer l'embedding des recettes, nous avons procédé de la manière suivante :

1. Charger les données du JSON
2. Créer un histogramme qui compte le nombre d'occurrences de chaque mot
3. Pour chaque recette, prendre le nom et les ingrédients et créer l'embedding de la manière suivante :
...
4. Moyenner l'emmbeding de la manière suivante :
    ...
    * Si l'embedding des ingrédients est vide → moyenner seulement l'embedding du nom
    * Si l'embedding du nom est vide → moyenner seulement l'embedding des ingrédients
    * Sinon → moyenner l'embedding des ingrédients en additionnant l'embedding du titre en lui donnant un poids de 5
5. Sauvegarder en JSON

Pour faire l'embedding des vins, nous avons effectué les mêmes étapes en retirant la création de l'histogramme. Ces différents embeddings sont donc calculés et mis dans la base de donnée. Ce sont des données statiques qui ne vont pas changer dans le futur.
    
Pour ce qui est du calcul de la similarité entre un embedding d'une recette et les embeddings des vins, cette étape est faite dynamiquement. Lorsque l'ont choisi une recette, une liste de quelques vins (1, 5, 10 ou 20) est affiché. Ces vins sont ceux qui ont la plus grande similarité.
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/diagrams_embeddings.png?raw=true" align="center" width="100%">
</p>
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/images/sentiment_analysis.png?raw=true" align="center" width="100%">
</p>
...
<p align="center">
  <img src="https://github.com/MSE-WEM/wine-recommender/blob/main/docs/gantt/gantt.svg?raw=true" align="center" width="100%">
</p>
...
Pour l'architecture, on pourrait ajouter des sécurités sur l'API afin de ne pas avoir accès directement à la DB.
